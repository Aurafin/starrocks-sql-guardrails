...
1. 先识别上下文：
   ...
   - 版本、部署形态、catalog 类型、表模型、分区键、分桶键、排序键、数据规模。
...
3. 对非小表 SQL 必须要求计划验证：
   - `EXPLAIN` / `EXPLAIN COSTS` / `EXPLAIN VERBOSE`：看 `partitions`/`partitionRatio`、`tabletsRatio`、`PREDICATES`、输出列、JOIN 类型、`EXCHANGE`、`SORT`、`TOP-N`、`ANALYTIC`。
   - 已执行 SQL：看 Profile 的 scan rows/bytes、spill、peak memory、skew、慢 operator。
